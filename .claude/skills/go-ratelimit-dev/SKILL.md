...
- `cmd/bench/` — **тонкий** харнесс: флаги, драйвер нагрузки, рендер таблицы.
  Никакой алгоритмической логики в `main`. Разложен на три файла одного
  `package main`: `main.go` (флаги, валидация, `-algo` → конструктор),
  `runner.go` (драйвер нагрузки), `report.go` (гистограмма и рендереры).
- Заглушки Этапа 0 помечались `TODO(Этап N)` и содержали `panic(...)` в теле +
  compile-time assertion `var _ Limiter = (*T)(nil)`. В коде MVP заглушек уже не
  осталось; правило действует на будущие этапы: при реализации **заменяй
...
- **Под тем же запретом — «локальный счётчик + одна блокировка в конце»**: копить
  допуски в переменной внутри горутины и слить их под мьютексом (или через один
  `atomic.Add`) после цикла. Формально это не `int` под mutex, и в обычном коде
  такой приём правильный — счётчик не становится точкой contention. Но в
  стресс-тесте лимитера цель обратная: contention — это и есть предмет
  измерения. Локальное накопление снимает с горутин ровно то давление на общее
  состояние, ради которого тест написан, и ослабляет его тем сильнее, чем больше
  `callsPer`. Считать `allowed.Add(1)` на каждый допуск — честнее и короче.
  Так были написаны три `*_ConcurrentNoOverAdmit` до PR #5; в стресс-тестах
  Этапа 4 паттерн не воспроизводить.
...
- Бенчмарки — по одному паттерну на все алгоритмы, чтобы сравнение было честным
  ...
  ориентированный харнесс поверх тех же лимитеров, печатает сравнительную
  таблицу для README. Флаги: `-algo` (`token`/`sliding`/`leaky`/`lockfree`/
  `all`, дефолт `all`), `-goroutines`, `-rate`, `-capacity`, `-duration`,
  `-format` (`text`/`markdown`), `-percentiles`.
- **Сьют меряет только путь допуска, и это осознанно.** `rate`/`capacity` по
  умолчанию заведомо выше нагрузки: на непустом бакете сравнение mutex против
  CAS осмысленно, на пустом — нет (lock-free возвращает `false` без CAS и без
  аллокации, mutex-версии и на отказе берут блокировку). Поэтому `cmd/bench`
  печатает warning, как только в любой строке `denied > 0`. Не «чинить» это
  подкруткой конфигурации.
