...
отказ возвращает `false` **без CAS** — это сознательное расхождение с
mutex-версией, которая персистит refill и на отказе. Обоснование обоих решений
— в doc-комментариях `lockfree_tokenbucket.go`, разбор — в
`docs/plans/stage-4-lockfree-tokenbucket.md`. Побочный эффект «деная без CAS»:
дрейф float64 в этой реализации структурно не копится, поэтому `admitEpsilon`
здесь закреплён отдельным unit-тестом на чистую функцию, а не сценарным
тестом, как у соседей.
...
| `internal/limiter/*_test.go` | По файлу тестов на алгоритм |
